...
Screen(address, refresh_rate):
    drawCircle(centre, radius, width)
    drawFOV(centre, radius, direction, angle) [degrees]
    drawCross(centre, radius)
    drawVector(centre, magnitude, direction [degrees])
    clear()
    update()
    start(update_function: Callable) -> You can pass a custom draw function into this loop.
...
async def main(camera, screen):
    ...
    while True:
        ...
        movement_direction = ts.calculateFinalDirection(screen.ballAngle, screen.ballDistance)
